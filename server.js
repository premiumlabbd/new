...
// Desktop headers (original site will recognize as desktop)
const BASE_HEADERS = {
...
// Axios instance with timeout 0 and optional proxy
function createAxiosInstance(cookies = [], proxy = null) {
const config = {
headers: { ...BASE_HEADERS },
timeout: 0, // wait indefinitely
};
if (cookies.length) config.headers['Cookie'] = cookies.join('; ');
if (proxy) config.proxy = proxy; // { host: 'IP', port: 8080 }
...
if (res.status === 302 && res.headers.location.includes('mov-verification')) {
    ...
    return { session: createAxiosInstance(cookies), cookies };
}
...
const res = await session.get(url, { headers, timeout: 0 });
return res.data;
}
...
features: { desktopUA: true, concurrentOTP: true, improvedPasswordGeneration: true, mobilePrefix: MOBILE_PREFIX }
});
});
