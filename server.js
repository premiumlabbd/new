...
const MAX_CONCURRENT = 20; // Vercel-compatible concurrency
...
async function axiosRetry(config, retries = 3) {
...
    const res = await axiosRetry({ method: 'post', url, data, headers, maxRedirects: 0, validateStatus: null, timeout: 0 });
...
app.get('/health', (req, res) => res.json({ status: 'OK', timestamp: new Date().toISOString(), service: 'Enhanced NID Info API', version: '2.0.5' }));
